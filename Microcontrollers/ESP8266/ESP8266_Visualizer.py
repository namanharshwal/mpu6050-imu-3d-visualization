#!/usr/bin/env python3
"""
ESP8266 MPU6050 3D Visualizer
Real-time 3D STL model visualization with serial IMU data from NodeMCU v3
...
BAUD_RATE = 115200            # Must match ESP8266 firmware
...
def init_serial():
    """
    Initialize serial communication with NodeMCU
    """
    global ser
    try:
        ...
        time.sleep(2)  # Wait for ESP8266 to stabilize
        return True
    except serial.SerialException as e:
        print(f"[ERROR] Failed to open serial port {SERIAL_PORT}")
        print(f"[ERROR] Details: {e}")
        print(f"[INFO] Check if NodeMCU is connected and port is correct")
        return False
...
def read_serial_data():
    """
    Read and parse JSON data from ESP8266
    """
    global roll, pitch, yaw
...
def update_fps():
    ...
    if current_time - last_fps_time >= 1.0:
        print(f"FPS: {fps_counter} | Roll: {roll:.2f}° | Pitch: {pitch:.2f}° | Yaw: {yaw:.2f}°")
        fps_counter = 0
...
def main():
    ...
    print("\n" + "="*60)
    print("  ESP8266 NodeMCU v3 - MPU6050 3D Visualizer")
    print("="*60)
    ...
    pygame.display.set_caption("ESP8266 MPU6050 - 3D Orientation Visualizer")
